...
```{r}
write.csv(clinic, "/home1/sskatta/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```
...
The data type of $gender is chr (character).

# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)?

The age at diagnosis of the 5th patient was 50 years old.
...
The age at diagnosis of the 1st patient was 55 years old. The initial pathologic diagnosis year was 2004, and the form completion date was 2014. This means the patient would be 65 years old at their last follow up. Additionally, adding 20211 (from birth_days_to) and 3767 (last_contact_days_to) and dividing by 365 nets 65 years.
...
plot(clinic$age_at_diagnosis, clinic$lymph_nodes_examined_count,
     main = "Lymph Nodes Examined Count vs Age at Diagnosis",
...
```{r Exercise 4.2}
# create plot here
values_contact <- as.numeric(clinic$last_contact_days_to)
boxplot(values_contact ~ clinic$gender,
        main = "Boxplot of last contact days to based on gender",
        xlab = "Last Contact Days To",
        ylab = "Gender",
        notch = TRUE,
        col = "red")
```
...
The y axis represents the survival probability from 0 to 1 from the clinical data.

# 2. What does the x axis represent?

The x axis represents the time of survival in days from the clinical data.

# 3. What age group appears to have the greatest rate of survival?

The age group with the greatest rate of survival appears to be the middle category. However, these curves all look relatively similar up to a certain point.

# 4. What age group appears to have the worst rate of survival? 

The age group with the worst rate of survival appears to be the young category. However, these curves all look relatively similar up to a certain point.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?

The approximate 15 year (5475 days) survival rate for "Old" breast cancer patients is about ~0.3 or 30%.
...
The p-value is 0.2, which is not statistically significant as it is not less than 0.05. This tells us that we can not reject the null hypothesis that there is no relationship between age and breast cancer survival. This makes sense given how the curves across the age groups looked quite similar.
